...
func GenerateBinaryBuild(siteCollection models.SiteCollection, config *config.Config) error {
	...
	cmd := exec.Command("sh", "-c", fmt.Sprintf("cd %s && git reset --hard origin/%s && git fetch origin && git checkout %s && git pull origin %s", config.Manager.AppsDir, GitBranch, GitBranch, GitBranch))
